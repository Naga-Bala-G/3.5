...
def change_role():
    ...
    try:
        data = request.json
        username = data.get('username')
        old_role = data.get('old_role')  # Old role to remove
        new_role = data.get('new_role')  # New role to assign
        ...
        # Introspect the access token to verify its validity
        logging.info("Introspecting access token")
        introspect_response = requests.post(f'{KEYCLOAK_SERVER_URL}/realms/{REALM_NAME}/protocol/openid-connect/token/introspect', data={
            'token': access_token,
            'client_id': CLIENT_ID,
        ...
        if 'admin' not in roles:
            logging.warning("Requester does not have admin role")
            abort(403, description='Forbidden')

        logging.info(f"Changing role for user {username} from {old_role} to {new_role}")
        ...
        # Fetch the role ID for the old role (to be removed)
        logging.info(f"Fetching role ID for old role {old_role}")
        old_role_response = requests.get(f'{KEYCLOAK_SERVER_URL}/admin/realms/{REALM_NAME}/roles/{old_role}', headers={
            'Authorization': f'Bearer {access_token}',
            'Content-Type': 'application/json'
        })

        if old_role_response.status_code != 200:
            logging.error(f"Failed to fetch role ID for {old_role}")
            abort(old_role_response.status_code, description='Failed to fetch old role ID')

        old_role_data = old_role_response.json()
        old_role_id = old_role_data['id']

        # Fetch the role ID for the new role (to be assigned)
        logging.info(f"Fetching role ID for new role {new_role}")
        new_role_response = requests.get(f'{KEYCLOAK_SERVER_URL}/admin/realms/{REALM_NAME}/roles/{new_role}', headers={
            'Authorization': f'Bearer {access_token}',
            'Content-Type': 'application/json'
        })

        if new_role_response.status_code != 200:
            logging.error(f"Failed to fetch role ID for {new_role}")
            abort(new_role_response.status_code, description='Failed to fetch new role ID')

        new_role_data = new_role_response.json()
        new_role_id = new_role_data['id']

        # Remove the old role from the user
        logging.info(f"Removing role {old_role} from user {username}")
        ...
        if remove_role_response.status_code != 204:
            logging.error(f"Failed to remove role {old_role} from user {username}")
            abort(remove_role_response.status_code, description='Failed to remove old role')

        # Assign the new role to the user
        logging.info(f"Assigning new role {new_role} to user {username}")
        ...
        if assign_role_response.status_code != 204:
            logging.error(f"Failed to assign new role {new_role} to user {username}")
            abort(assign_role_response.status_code, description='Failed to assign new role')

        logging.info(f"Role changed from {old_role} to {new_role} for user {username}")
...
def remove_role():
    ...
    try:
        ...
        logging.info(f"Removing role {role_name} from user {username}")
        ...
        # Fetch the role by name to get the role ID
        logging.info(f"Fetching role ID for role {role_name}")
        ...
        if remove_role_response.status_code != 204:
            logging.error(f"Failed to remove role {role_name} from user {username}")
            abort(remove_role_response.status_code, description='Failed to remove role')

        logging.info(f"Role {role_name} removed from user {username} successfully")
...
def assign_role():
    ...
    try:
        ...
        logging.info(f"Assigning role {role_name} to user {username}")
        ...
        if assign_role_response.status_code != 204:
            logging.error(f"Failed to assign role {role_name} to user {username}")
            abort(assign_role_response.status_code, description='Failed to assign role')

        logging.info(f"Role {role_name} assigned to user {username} successfully")
